...
int findDist(int start_index, int destination_index, int num_places,
   node_t places[num_places], list_entry_t* paths[], int heap[], int* heap_size);
int findIndex(char place[], node_t places[], hashtable_entry_t* hashtable[]);
static void heap_shift_up(int heap[], int heap_index, node_t places[]);
static void heap_shift_down(int heap[], int heap_index, int* heap_size, node_t places[]);
static void heap_insert(int heap_entry, int heap[], int* heap_size, node_t places[]);
static int extract_heap_min(int heap[], int* heap_size, node_t places[]);
...
int main (int argc, char* argv[])
{
  ...
  node_t places[num_places]; // make node_t* ?? - means dont need num places in function definitions
...
int findDist(int start_index, int destination_index, int num_places,
   node_t places[num_places]/*remove num_places??*/, list_entry_t* paths[], int heap[], int* heap_size)
{
  int num_loops = 0;
  ...
  while (1)
  {
    ...
    while (entry != NULL)
    {
      ...
      printf("\nentry->index: %d\n", entry->index);
      printf("places[entry->index].fixed: %d\n\n", places[entry->index].fixed);
      if (entry->dist > 0 && places[entry->index].fixed == false)
      {
          ...
          || places[entry->index].dist == 0)
        {
          ...
          printf("heap_size: %d\n", *heap_size);
          heap_insert(entry->index, heap, heap_size, places); //make heap_size an int pointer??
        }
      }
      entry = entry->next;
    }
    ...
    //find smallest
    int smallest_dist_index = extract_heap_min(heap, heap_size, places);
    printf("smallest: %d\n", smallest_dist_index);
    /*for (int i = 0; i < num_places; i++)
    {
...
static void heap_shift_up(int heap[], int heap_index, node_t places[])
{
  while (heap_index > 1)
  {
    int parent_place_index = heap[heap_index / 2];
    int child_place_index = heap[heap_index];
    if (places[parent_place_index].dist > places[child_place_index].dist)
    {
      places[parent_place_index].heap_index = heap_index;
      places[child_place_index].heap_index = heap_index / 2;
...
static void heap_shift_down(int heap[], int heap_index, int* heap_size, node_t places[])
{
  ...
  {
    int child_place_index = heap[heap_index * 2];
    int parent_place_index = heap[heap_index];
    if (places[parent_place_index].dist > places[child_place_index].dist)
    {
      places[child_place_index].heap_index = heap_index;
      places[parent_place_index].heap_index = heap_index * 2;
...
static void heap_insert(int heap_entry, int heap[], int* heap_size, node_t places[])
{
  heap[*heap_size] = places[heap_entry].dist;
  places[heap_entry].heap_index = *heap_size;
  heap_shift_up(heap, *heap_size, places);
  (*heap_size)++;
}
...
static int extract_heap_min(int heap[], int* heap_size, node_t places[])
{
  ...
  (*heap_size)--;
  heap_shift_down(heap, 1, heap_size, places);
  return min;
}
